...
var Expr = (function () {
                ...
                if (httpRequest !== null) {
                    ...
                    if (!context.fixed.documentPool[abs]) {
                        SaxonJS.getPlatform().asyncGetXml(abs, function (uri, doc) {
                            //saxonPrint("Added document to pool " + uri);
                            context.fixed.documentPool[uri] = doc;
                            fn();
                        });
                    } else {
                        //saxonPrint("Found " + abs + " in pool");
                        fn();
                    }
                } else if (wait !== 0) {
                    setTimeout(fn, wait);
    ...
    function serialize(seq, params) {
        ...
        if (!params) {
        } else if (params instanceof Iter.Singleton) {
...
var CoreFn = (function () {
    ...
    return {
        ...
        "error": function (args) {
            ...
            code = code.local;
            var desc = stringZeroLengthIfAbsent(args[1]);
            //TODO add arguments
            ...
            return args[0].mapOneToOne(function (item) {
                ...
                Axis.ancestor(item).forEachItem(function (anc) {
                    ...
                    Axis.attribute(anc).forEachItem(function (att) {
                        ancCopy.setAttributeNS(att.namespaceURI, att.name, att.value);
                    });
